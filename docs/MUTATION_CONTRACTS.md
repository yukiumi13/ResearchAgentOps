...
| `researchctl github configure-governance` | Manager only; protected Project policy owns the accepted non-secret GitHub identity and merge-gate contract | `OperationStarted`, then one canonical ProjectPolicy commit on a fixed control branch/worktree | Operation ID + exact default head + explicit GitHub policy digest | Re-read exact base, fixed branch/marker/path, and both ProjectPolicy records; retry accepts only the same `github` replacement, and CI proves every other field unchanged | `proposal_prepared` or `no_change`; acceptance requires exact-head CI, CODEOWNER review, and protected merge |
| `researchctl github apply-governance` | Read-only for preview; explicit mutation requires a Manager shell plus authenticated allowlisted human GitHub user/team | One bounded classic branch-protection API update after all local/remote preconditions | Accepted policy digest + immediately observed governance digest | Preview is non-mutating; apply re-observes before digest checks, rejects Session/App/ruleset/bypass ambiguity, sends one canonical payload, then audits through the same observer; timeout or bad read-back is not success | `preview`, `no_change`, `applied`, or a failed/uncertain/incomplete error requiring a fresh preview |
...
| `researchctl-github-proposal-host submit` with strict `SubmissionCreateRequest` | Assigned Agent proposes through a Session capability; the App-authored submission branch/PR owns the proposal | `OperationStarted`, then submission branch commit before PR creation | Submission ID + base SHA + ordered RunResult digests + claim digest | Reauthenticate Session, verify App/repository/permissions, then observe branch, deterministic bundle, push, and PR; local/config/size/digest failures occur before remote mutation | `proposal_open`, `withdrawn`, `failed`, or `canceled`; never `accepted` |
