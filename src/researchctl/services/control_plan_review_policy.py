...
from researchctl.domain.models import PlanReviewPolicy, ProjectPolicy
...
class PlanReviewPolicyWriteResult:
...
class ControlPlanReviewPolicyRepository:
    """Prepare one manager-owned Plan reviewer policy proposal."""
    ...
    def __init__(
        ...
        *,
        ...
        GitControlCommitAdapter.validate_plan_review_identity(
        ...
        if not isinstance(default_branch, str) or not default_branch:
            raise RCPError(
                ...
                message="Plan review policy control requires a default branch.",
            )
    ...
    def configure(
        ...
        with self._exclusive():
            ...
            if base != self.expected_default_head:
                raise RCPError(
                    code="control_plan_review_base_invalid",
                    message="Plan review policy base did not resolve exactly.",
                )
            base_content = self.commits.project_policy_content_at(
                repository_root=self.repository_root,
                commit=base,
            )
            if base_content is None:
                raise RCPError(
                    code="control_plan_review_policy_missing",
                    message="The protected base has no Project policy to update.",
                )
            previous = self._project_policy(base_content)
            replacement = previous.model_copy(update={"plan_review": review_policy})
            ...
            if branch_head is None:
                ...
                changed = self.commits.validate_plan_review_operation_head(
                    repository_root=self.repository_root,
                    commit=branch_head,
                ...
                if changed:
                    branch_content = self.commits.project_policy_content_at(
                        repository_root=self.repository_root,
                        commit=branch_head,
                    )
                    if (
                        ...
                        raise RCPError(
                            code="control_plan_review_retry_mismatch",
                            message=(
                                "Existing Plan review policy proposal differs from "
                                "this idempotent request."
                            ),
                        )

            self._prepare_worktree(branch_head)
            self.commits.require_plan_review_worktree_scope(
            ...
            proposal = self.commits.commit_plan_review_policy_or_observe(
                worktree=self.worktree,
                branch=self.branch,
                operation_id=self.operation_id,
                expected_base_commit=base,
            )
            return PlanReviewPolicyWriteResult(
    ...
    def _exclusive(self) -> Iterator[None]:
        lock = self.worktrees_directory / f".plan-review-configure-{self.operation_id}.lock"
        flags = os.O_RDWR | os.O_CREAT | getattr(os, "O_NOFOLLOW", 0)
        try:
            descriptor = os.open(lock, flags, 0o600)
        except OSError as error:
            raise RCPError(
                code="control_plan_review_lock_invalid",
                message="Plan review policy lock could not be opened safely.",
            ) from error
        try:
    ...
    def _require_current_default(self) -> None:
        ...
        if observed != self.expected_default_head:
            raise RCPError(
                code="control_plan_review_default_head_changed",
                message="Default branch no longer matches the Plan review policy base.",
                context={
    ...
    def _prepare_worktree(self, branch_head: str) -> None:
        ...
        try:
            ...
            if not branch_only:
                raise
            self.commits.attach_existing_plan_review_branch(
                repository_root=self.repository_root,
                worktree=self.worktree,
                branch=self.branch,
    ...
    def _read_current(path: Path) -> bytes:
        if path.is_symlink() or not path.is_file():
            raise RCPError(
                code="control_plan_review_policy_path_invalid",
    ...
    def _project_policy(content: bytes) -> ProjectPolicy:
        try:
            ...
            raise RCPError(
                code="control_plan_review_policy_invalid",
    ...
    def _write_atomic(path: Path, content: bytes) -> None:
        ...
        descriptor, name = tempfile.mkstemp(prefix=".plan-review.", dir=parent)
        temporary = Path(name)
        try:
            ...
            raise RCPError(
                code="control_plan_review_policy_write_failed",
