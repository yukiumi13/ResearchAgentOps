...
from researchctl.domain.models import GitHubGovernancePolicy, ProjectPolicy
...
_COMMAND = "github.configure-governance"


@dataclass(frozen=True, slots=True)
class GitHubGovernancePolicyWriteResult:
...
class ControlGitHubGovernancePolicyRepository:
    """Prepare one manager-owned GitHub governance policy proposal."""
        ...
        *,
        ...
        if not isinstance(default_branch, str) or not default_branch:
            raise RCPError(
                ...
                message="GitHub governance control requires a default branch.",
            )
    ...
    def configure(
        ...
        with self._exclusive():
            ...
            if base != self.expected_default_head:
                raise RCPError(
                    code="control_github_governance_base_invalid",
                    message="GitHub governance policy base did not resolve exactly.",
                )
            ...
            if base_content is None:
                raise RCPError(
                    code="control_github_governance_policy_missing",
                    message="The protected base has no Project policy to update.",
                )
            previous = self._project_policy(base_content)
            replacement = previous.model_copy(update={"github": governance})
            ...
            if branch_head is None:
                ...
                if changed:
                    ...
                    if (
                        ...
                        raise RCPError(
                            code="control_github_governance_retry_mismatch",
                            message=(
                                "Existing GitHub governance proposal differs from "
            ...
            return GitHubGovernancePolicyWriteResult(
    ...
    def _exclusive(self) -> Iterator[None]:
        lock = self.worktrees_directory / f".github-governance-{self.operation_id}.lock"
        flags = os.O_RDWR | os.O_CREAT | getattr(os, "O_NOFOLLOW", 0)
        try:
            descriptor = os.open(lock, flags, 0o600)
        except OSError as error:
            raise RCPError(
                code="control_github_governance_lock_invalid",
                message="GitHub governance policy lock could not be opened safely.",
            ) from error
        try:
    ...
    def _require_current_default(self) -> None:
        ...
        if observed != self.expected_default_head:
            raise RCPError(
                code="control_github_governance_default_head_changed",
                message="Default branch no longer matches the GitHub governance base.",
                context={
    ...
    def _read_current(path: Path) -> bytes:
        if path.is_symlink() or not path.is_file():
            raise RCPError(
                code="control_github_governance_policy_path_invalid",
    ...
    def _project_policy(content: bytes) -> ProjectPolicy:
        try:
            ...
            raise RCPError(
                code="control_github_governance_policy_invalid",
    ...
    def _write_atomic(path: Path, content: bytes) -> None:
        ...
        descriptor, name = tempfile.mkstemp(prefix=".github-governance.", dir=parent)
        temporary = Path(name)
        try:
            ...
            raise RCPError(
                code="control_github_governance_policy_write_failed",
